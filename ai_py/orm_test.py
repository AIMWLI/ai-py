...
def sql_test(request):
    ...
    查询姓“李”的老师的个数；
    '''
    teacher_nums = Teacher.objects.filter(name__startswith="z").count()
    print(teacher_nums)
    # if rows != None:
    #     return HttpResponse(teacher_nums)
    '''
    查询没学过“李老师”课的同学的id、姓名；
    '''
    rows = Student.objects.exclude(score__course__teacher__name="hel").values('id', 'name')
    for row in rows:
        print(row)
    '''
    ...
    '''
    rows = Student.objects.annotate(nums=Count("score__course", filter=Q(score__course__teacher__name='hel')))\
        .filter(nums=Course.objects.filter(teacher__name='hel').count()).values('id', 'name')
    for row in rows:
        print(row)
    '''
